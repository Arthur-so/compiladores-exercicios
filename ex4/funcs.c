...
char get_next_char(FILE *fptr, Buffer* buffer) {
    // caso esteja no limite do buffer ou no final da string armazenada no buffer
    if (buffer->next_char_id >= buffer->size || buffer->data[buffer->next_char_id] == '\0') {
        ...
        // caso exista um sinal de nova linha
        if (buffer->new_line) {
    ...
    if (buffer->data[buffer->next_char_id] == '\n') { // Nova linha
        buffer->new_line = 1; // levanta snal de nova linha
    }
